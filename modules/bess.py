...
def add_bat(Bess):
    ...
    new_bat = f"New Storage.Bess_{name}_bus_{bus} bus1=bus_{bus} State={state} Phases={Phases} kV={kV} kW={kW} kWrated={Pmax} kWhRated={Emax} %stored={SOC} %EffCharge={Eff} %IdlingkW=0 %EffDischarge={Eff} %reserve={soc_min} %X=0"
